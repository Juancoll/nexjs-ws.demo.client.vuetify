...
export class VueRouterExtended extends Router {
    ...
    public get isAuthRoute(): boolean {
        if (
            !this._branches || !this._branches.authenticate || !this._branches.authenticate.auth
        ) {
            return false;
        }
    ...
    constructor(props: IVueRouterExtendedProps) {
        ...
        (this as any).beforeEach((to: Route, from: Route, next: any) => {
            if (self._firstTime) {
                ...
                if (!self._currentBranch) {
                    throw new Error('current Branch is undefined');
                }
    ...
    public setBranches(branches: IBranches) {
        ...
        if (
            this._branches.authenticate && this._branches.authenticate.commonForRoles
        ) {
    ...
    public get userValidRoles(): string[] {
        return !this._user || !this._user.roles ? [] : this._user.roles.filter((x) => this.roles.indexOf(x) != -1);
    }
    ...
    public updateRole(role?: string) {
            ...
            if (!this._user) {
                ...
                if (!this.useRoles) {
                    if (!this._branches.authenticate.anyRole) {
                        throw new Error('"authenticate.anyRole" branch required');
                    }
                    ...
                    if (!this._branches.authenticate.roles) {
                        throw new Error('"authenticate.roles" branch required ');
                    }
        ...
        this.push({ path: this._currentBranch.startup }).catch((e) => {
            console.log(
                `[router] try to push existing current path = ${
                this._currentBranch ? this._currentBranch.startup : "currentBranch = undefined"
                }`
            );
        });
    }
